    ...
    {loading
      ? Array.from({ length: 6 }).map((_, i) => (
          <div key={i} className="flex flex-col sm:grid grid-cols-12 gap-2 p-4 border-b animate-pulse" style={{background: 'var(--card-bg)', borderColor: 'var(--border)'}}>
            ...
            <div className="col-span-3 h-4 bg-[#e6f9f5] rounded mb-2" />
            <div className="col-span-3 h-4 bg-[#e6f9f5] rounded mb-2" />
      ...
      : sources.map((source) => (
          <div key={source.id || source.url || source.title} className="flex flex-col sm:grid grid-cols-12 gap-2 p-4 border-b" style={{background: 'var(--card-bg)', borderColor: 'var(--border)'}}>
            ...
            <div className="col-span-3 text-xs text-[#7ca982]">{source.published_on ? new Date(source.published_on).toLocaleDateString() : '-'}</div>
            <div className="col-span-3 text-xs text-[#7ca982]">{source.updated_on ? new Date(source.updated_on).toLocaleDateString() : '-'}</div>
